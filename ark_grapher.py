...
def get_institutional_holders(ticks):
    ...
    df1 = pd.DataFrame.from_dict(institutional_holders, index=[0])
    st.bar_chart(df1)

    st.write("% Of Shares held by Institutions by stock")
    df2 = pd.DataFrame.from_dict(institutional_shares, index=[0])
    st.bar_chart(df2)

    st.write("% Of Shares held by Insiders by stock")
    df3 = pd.DataFrame.from_dict(insider_shares, index=[0])
    st.bar_chart(df3)
